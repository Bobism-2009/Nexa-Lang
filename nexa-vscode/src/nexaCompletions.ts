...
export const MODULE_MEMBERS: Record<string, { name: string; detail: string }[]> = {
  ...
  gfx3d: [
    ...
    { name: "box", detail: "gfx3d.box(x, y, z, w, h, d, r, g, b) — three different sides" },
    { name: "sphere", detail: "gfx3d.sphere(x, y, z, radius, r, g, b) — smooth-shaded" },
    { name: "capsule", detail: "gfx3d.capsule(x1,y1,z1, x2,y2,z2, radius, r,g,b) — rounded ends" },
    { name: "cylinder", detail: "gfx3d.cylinder(x1,y1,z1, x2,y2,z2, radius, r,g,b) — flat ends" },
    { name: "cone", detail: "gfx3d.cone(x1,y1,z1, x2,y2,z2, radius, r,g,b) — tip at the second point" },
    { name: "line3", detail: "gfx3d.line3(x1,y1,z1, x2,y2,z2, r,g,b) — unshaded" },
    { name: "grid", detail: "gfx3d.grid(size, step, r, g, b) — lines on the y = 0 plane" },
    { name: "translate", detail: "gfx3d.translate(x, y, z) — moves what is drawn next" },
    { name: "rotate", detail: "gfx3d.rotate(rx, ry, rz) — degrees, right-hand rule, X then Y then Z" },
    { name: "scale", detail: "gfx3d.scale(s) — uniform" },
    { name: "reset", detail: "gfx3d.reset() — transform back to world space" },
    { name: "light", detail: "gfx3d.light(x, y, z[, r, g, b]) — direction it shines from" },
    { name: "ambient", detail: "gfx3d.ambient() / gfx3d.ambient(n) — 0..255 facing away; default 87" },
    { name: "key", detail: "gfx3d.key(name) — 1 while that key is down" },
    { name: "pressed", detail: "gfx3d.pressed(name) — 1 on the frame it went down" },
    { name: "released", detail: "gfx3d.released(name) — 1 on the frame it came up" },
    { name: "typed", detail: "gfx3d.typed() — text typed since the last poll" },
    { name: "wheel", detail: "gfx3d.wheel() — vertical scroll notches since the last poll" },
    { name: "wheel_x", detail: "gfx3d.wheel_x() — the same, horizontal" },
    { name: "mouse", detail: "gfx3d.mouse(button) — left / right / middle" },
    { name: "mouse_x", detail: "gfx3d.mouse_x() — cursor in window pixels" },
    { name: "mouse_y", detail: "gfx3d.mouse_y() — cursor in window pixels" },
    { name: "model", detail: "gfx3d.model(path) — load a Wavefront .obj, returns handle or 0" },
    { name: "draw", detail: "gfx3d.draw(id, x, y, z, scale, r, g, b) — draw a model; scale is world units" },
    { name: "model_tris", detail: "gfx3d.model_tris(id) — triangles the file parsed to" },
    { name: "sound", detail: "gfx3d.sound(path) — load a WAV, returns handle or 0 (std/gfx's mixer)" },
    { name: "play", detail: "gfx3d.play(id[, volume]) — play once, returns a voice or 0" },
    { name: "loop", detail: "gfx3d.loop(id[, volume]) — play repeating, returns a voice or 0" },
    { name: "stop", detail: "gfx3d.stop() / gfx3d.stop(voice) — stop every voice, or one" },
    { name: "volume", detail: "gfx3d.volume() / gfx3d.volume(v) — get or set master volume 0..255" },
    { name: "audio", detail: "gfx3d.audio([rate]) — open 16-bit mono PCM (default 44100)" },
    { name: "sample", detail: "gfx3d.sample(s) — queue one s16 sample" },
    { name: "audio_queued", detail: "gfx3d.audio_queued() — samples still buffered" },
    { name: "audio_flush", detail: "gfx3d.audio_flush() — submit a partial PCM buffer" },
  ],
  gfx: [

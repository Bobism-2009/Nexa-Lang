...
//
// The names are __nexa_net_*, not the __nexa_sock_* they read as, because
// http.*'s listening half already defines a set of its own under that prefix
// in HttpRuntime.hpp. A program that serves HTTP and opens a raw socket emits
// both, so the two cannot share a name. Folding http.*'s copy into this one is
// the obvious next tidy-up; it is a live server's platform layer, so it is not
...
#include <winsock2.h>
#include <ws2tcpip.h>
typedef SOCKET __nexa_net_sock_t;
#define __NEXA_NET_BAD INVALID_SOCKET
#define __NEXA_NET_SEND_FLAGS 0
[[maybe_unused]] static void __nexa_net_shut(__nexa_net_sock_t s) { closesocket(s); }
...
[[maybe_unused]] static int __nexa_net_start() {
...
#include <netdb.h>
#include <unistd.h>
typedef int __nexa_net_sock_t;
#define __NEXA_NET_BAD (-1)
...
#ifdef MSG_NOSIGNAL
#define __NEXA_NET_SEND_FLAGS MSG_NOSIGNAL
#else
#define __NEXA_NET_SEND_FLAGS 0
#endif
[[maybe_unused]] static void __nexa_net_shut(__nexa_net_sock_t s) { close(s); }
[[maybe_unused]] static int __nexa_net_start() { return 1; }
#endif
[[maybe_unused]] static void __nexa_net_nosigpipe(__nexa_net_sock_t s) {
#ifdef SO_NOSIGPIPE
  int one = 1;
...
[[maybe_unused]] static int __nexa_net_handle(__nexa_net_sock_t s) {
  if (s == __NEXA_NET_BAD) return 0;
#ifndef _WIN32
  if (s == 0) {
    int moved = dup(s);
    close(s);
